function [dates, alpha1, alpha2, alpha3,alpha1hat, alpha2hat, alpha3hat, tau1, tau2, tau3, tau1hat, tau2hat, tau3hat, logbetarain,logbetatemp, betalongterm, Y0, Y,Yhat,RNdensity1, r2_logRN, exitflag2] = fit_model_RN(dataset, nknots, h, penalty, saveBool)
...
Y = zeros(timePts,1);
for i=1:(timePts-1)
	Y(i,1) = log(RNdensity(i+1,1));
end
Y(isinf(Y)) = 0;

% generating the vector RN(t+1)
...
% entire fit.
% Yhat = logRN+f1hat+f2hat+U*tau1hat+V*tau2hat+W*tau3hat; 
% resids = Y-Yhat;
...
if saveBool
    filename = strcat('Opt_Results_RN_', dataset);
    save(filename, 'nknots', 'm', 'n', 'penalty', 'h', 'dates', 'Wblt', 'Iblt', 'X', 'Y0','Y','Yhat','RNdensity1', 'alpha1', 'alpha1hat', 'alpha2', 'alpha2hat', 'alpha3', 'alpha3hat', 'tau1', 'tau1hat','tau2', 'tau2hat','tau3', 'tau3hat','logbetarain','logbetatemp','betalongterm', 'f2hat');
